...
var _compat = require('@solana/compat');
...
function toLegacyPublicKey(value) {
  if (value instanceof _web3js.PublicKey) {
    return value;
  }
  if (isKitTransactionSigner(value)) {
    return new (0, _web3js.PublicKey)(value.address);
  }
  return new (0, _web3js.PublicKey)(value.toString());
}
...
function createKitTransactionPlan(transaction, signers = []) {
  return {
    instructions: transaction.instructions.map(
      _compat.fromLegacyTransactionInstruction
    ),
    signers: collectKitTransactionSigners(signers)
  };
}
async function createKitSignerFromLegacyKeypair(keypair) {
  return _kit.createSignerFromKeyPair.call(void 0, await _compat.fromLegacyKeypair.call(void 0, keypair));
}
...
function collectFirstBuySigners(params) {
  return collectKitTransactionSigners(_optionalChain([params, 'optionalAccess', _29 => _29.buyer]));
}
function collectPartnerFirstBuySigners(params) {
  return collectKitTransactionSigners(_optionalChain([params, 'optionalAccess', _30 => _30.partner]));
}
function collectCreatorFirstBuySigners(params) {
  return collectKitTransactionSigners(_optionalChain([params, 'optionalAccess', _31 => _31.creator]));
}
...
var DynamicBondingCurveKitClient = class _DynamicBondingCurveKitClient {
  ...
  static fromRpcUrl(rpcUrl, commitment = "confirmed") {
    const legacyClient = DynamicBondingCurveClient.create(
      new (0, _web3js.Connection)(rpcUrl, commitment),
      commitment
    );
    const rpc = _kit.createSolanaRpc.call(void 0, rpcUrl);
    return new _DynamicBondingCurveKitClient(legacyClient, rpc);
