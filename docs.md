...
Use `@meteora-ag/dynamic-bonding-curve-sdk/kit` when you want the Kit-native, zero-`@solana/web3.js` entrypoint.

- `@meteora-ag/dynamic-bonding-curve-sdk/kit` is the native Kit path. It exposes `DynamicBondingCurveKitClient.fromRpcUrl(...)` and `DynamicBondingCurveKitClient.fromRpc(...)`.
- The package root remains mixed-mode for backward compatibility. Root `DynamicBondingCurveKitClient` still supports `fromLegacyClient(...)` and `fromRpcUrl(rpcUrl, commitment)`, but it is not the zero-web3 import path.
...
The package root is still additive and legacy-compatible. Use the `./kit` subpath when you specifically need the Kit-native entrypoint.
